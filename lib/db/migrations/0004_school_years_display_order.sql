-- MIGRATE-DATA: populates display_order on existing school_year rows; mirrored by ensureSchoolYears() which applies the same ROW_NUMBER() ordering when all rows have display_order = 0 and more than one row exists

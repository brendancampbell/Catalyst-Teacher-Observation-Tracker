...
- **Outbound integration secrets and service access** — Google OAuth credentials, session secret, database credentials, and Anthropic access. Compromise could enable impersonation, data extraction, or abuse of external services.
...
- **API to third-party services** — the server calls Google OAuth and Anthropic using privileged credentials. Requests crossing this boundary must be scoped and validated.
...
- Highest-risk code areas: `routes/auth.ts`, `routes/observations.ts`, `routes/ai.ts`, `routes/people.ts`, `lib/passport.ts`, `middleware/auth.ts`, `artifacts/catalyst-dashboard/src/pages/drafts.tsx`, and frontend rich-text/navigation sinks
- Public surfaces: `/api/auth/google`, `/api/auth/google/callback`, `/api/app`
- Authenticated surfaces: dashboard, teacher, observation, people, AI, impersonation, admin-school routes
- Expensive external-service surfaces: `/api/ai/chat`, `/api/ai/chat/stream`, `/api/ai/analysis`, `/api/ai/school-summary`, `/api/qualitative-themes/generate`
...
Observation history, rubric notes, teacher/staff contact details, and network-wide analytics are sensitive personnel data. Every read path and every outbound sharing path must enforce school/network scope server-side, and rendered rich text or error responses must not leak more data than the caller is entitled to access. Historical observations must remain bound to the school and publication state they were created under; a later teacher transfer or AI aggregation helper must not surface prior-school records or another observer's drafts. The dashboard builds observation-feedback email HTML client-side for copy/paste; every interpolated field must be escaped and rich text sanitised so pasted messages cannot carry injected markup, links, or tracking images.

### Denial of Service

Public auth entry points and authenticated AI endpoints can trigger external-service work. The application must avoid unbounded expensive operations, especially on AI generation, and should not allow an attacker to create excessive sessions or outbound workload through repeated authenticated requests. Authenticated AI endpoints in particular should enforce per-user rate limits or quotas because each request can trigger substantial database reads plus Anthropic model usage.

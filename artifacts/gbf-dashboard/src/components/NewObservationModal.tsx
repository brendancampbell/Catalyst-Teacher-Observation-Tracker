...
export function NewObservationModal({ teachers, open, onOpenChange, onSubmit }: Props) {
  ...
  const [scores, setScores] = useState<Record<string, Score>>(
    () => Object.fromEntries(ALL_DOMAINS.map((d) => [d.id, 3 as Score])),
  );
  ...
  function reset() {
    ...
    setDate(todayIso);
    setScores(Object.fromEntries(ALL_DOMAINS.map((d) => [d.id, 3 as Score])));
    setStrengths("");
    setGrowthAreas("");
  }
  ...
  function handleSubmit() {
    if (!teacherId) return;
    onSubmit(teacherId, date, scores, strengths, growthAreas);
    reset();
    onOpenChange(false);
  }
        ...
        <DialogPrimitive.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-2xl max-h-[92vh] flex flex-col bg-white rounded-xl shadow-2xl overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95">
                ...
                <div className="border border-t-0 border-slate-200 rounded-b divide-y divide-slate-100">
                  {cat.domains.map((domain) => (
                    <div key={domain.id} className="flex items-center justify-between px-3 py-2.5">
                      <span className="text-sm font-medium text-slate-700 flex-1 pr-4">{domain.label}</span>
          ...
          <div className="shrink-0 px-6 py-4 border-t border-slate-200 flex justify-end gap-3 bg-slate-50">
            ...
            <button
              ...
              className="px-6 py-2 rounded text-sm font-bold text-white transition-opacity hover:opacity-90 shadow-sm"

...
/**
 * The rescore deadline is a setting now, so the assistant must not state a
 * number of its own. Substituted at call time rather than baked in: the
 * assistant confidently telling a coach the wrong deadline is worse than it
 * not knowing one.
 */
async function systemPrompt(): Promise<string> {
  const { rescoreWindowDays } = await getWindows();
  return CATALYST_SYSTEM_PROMPT.replace("{{RESCORE_WINDOW}}", describeWindow(rescoreWindowDays));
}
...
- "Rescore queue" means teachers who received a walkthrough score below 0.7 and need a follow-up observation within {{RESCORE_WINDOW}}.
